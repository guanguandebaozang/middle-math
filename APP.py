...
# 保留：哈希生成工具、题库上传、PDF导出、多用户数据隔离、错题本统计
# 适配：Streamlit Community Cloud 免费云端一键部署
...
# ====================== 【核心改造】云端SECRETS 账号读取（无硬编码） ======================
def init_authenticator():
    credentials = {"usernames": {}}
    # 动态读取云端所有账号
    try:
        sec = st.secrets["credentials"]
        for key in sec:
            if key.endswith("_name"):
                uname = key.replace("_name","")
                pwdkey = uname + "_pwd"
                if pwdkey in sec:
    ...
    authenticator = stauth.Authenticate(
        ...
        cookie_name="math_paper_cookie",
        key="math_paper_secure_key_2026",
...
# ====================== 主程序入口 ======================
def main():
    global authenticator, QUESTION_BANK
